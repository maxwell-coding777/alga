...
rules::rules(Uint16 type, board *b, Uint32 local_player_id) {
	...
	if (type == GAME2PMATCH || type == GAME2PDMATCH){
...
bool rules::valid_move() {
	...
	if (blobs.get(ox, oy) != Sint32(CURRENT_PLAYER)) {
		...
		cout<<"player "<<Sint32(CURRENT_PLAYER)<<" tries to move a blob of player "<<blobs.get(ox, oy)<<endl;
#endif
		return false;
	}
...
#ifdef DEBUG
	printf("Now fork: %s %s %s %s %s\n", "./launchStrategy", blobs.serialize().c_str(), holes.serialize().c_str(), cplayer.c_str(), std::to_string(strat).c_str());
#endif
	int childPid = fork();
	if(childPid == 0) // Child process
	{	
		execl("./launchStrategy", "./launchStrategy", blobs.serialize().c_str(), holes.serialize().c_str(), cplayer.c_str(), std::to_string(strat).c_str(), (char *)NULL);
	}
